...
struct TokenUsageScannerBreakdownTests {
    @Test
    func `codex daily report parses token counts and caches`() throws {
        ...
        let first = TokenUsageScanner.loadDailyReport(
            provider: .codex,
            since: day,
            until: day,
            now: day,
        ...
        let second = TokenUsageScanner.loadDailyReport(
            provider: .codex,
            since: day,
            until: day,
            now: day,
    ...
    func `codex first refresh keeps unrelated archived sessions out of cache`() throws {
        ...
        let cache = TokenUsageCacheIO.load(provider: .codex, cacheRoot: env.cacheRoot)
    ...
    func `codex root switch reloads long lived sessions from older partitions`() throws {
        ...
        var firstOptions = TokenUsageScanner.Options(
            codexSessionsRoot: env.codexSessionsRoot,
            claudeProjectsRoots: nil,
            cacheRoot: env.cacheRoot)
        firstOptions.refreshMinIntervalSeconds = 0

        _ = TokenUsageScanner.loadDailyReport(
            provider: .codex,
            since: reportDay,
            until: reportDay,
            now: reportDay,
        ...
        var secondOptions = TokenUsageScanner.Options(
    ...
    func `claude daily report parses usage and caches`() throws {
        ...
        let report = TokenUsageScanner.loadDailyReport(
            ...
            options: options)
        #expect(report.data.count == 1)
        #expect(report.data[0].modelsUsed == ["claude-sonnet-4"])
